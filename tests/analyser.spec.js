...
test.describe("Analyser", () => {
  ...
  test("should create an instance with custom AudioContext", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { Analyser } = await import("/dist/index.js");
...
test.describe("Monitor with Default Analyser", () => {
  ...
  test("should create Monitor with custom analyser", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { Monitor, Analyser, getMediaStream } = await import("/dist/index.js");
  ...
  test("should install audio worklet", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const workletNode = await monitor.installWorklet('test-worklet', workletUrl);
  ...
  test("should handle frequency data consistency", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        bothUint8Array: data1 instanceof Uint8Array && data2 instanceof Uint8Array,
        length: data1.length,
      };
    });
  ...
  test("should maintain separate monitor instances", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        bothHaveVolume: typeof monitor1.volume === "number" && typeof monitor2.volume === "number",
      };
    });
