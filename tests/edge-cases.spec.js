...
test.describe("Edge Cases", () => {
  ...
  test("should handle suspended AudioContext", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return {
        suspendedVolume: typeof volume1 === "number" && Number.isFinite(volume1),
  ...
  test("should handle multiple streams to single analyser", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { Analyser, getMediaStream } = await import("/dist/index.js");
      ...
      return {
        volumeValid: typeof volume === "number" && Number.isFinite(volume),
        frequencyDataValid: frequencyData instanceof Uint8Array && frequencyData.length > 0,
  ...
  test("should handle analyser with different sample rates", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { Analyser } = await import("/dist/index.js");
      ...
      for (const sampleRate of sampleRates) {
        try {
          ...
          oscillator.connect(analyser.node);
          oscillator.start();
          ...
          results.push({
            ...
            frequencyData: frequencyData instanceof Uint8Array && frequencyData.length > 0,
          });
        } catch (error) {
          results.push({
            sampleRate,
      ...
      return {
        results,
        successfulRates: results.filter(r => r.success).length,
        totalRates: results.length,
      };
    });

    expect(result.successfulRates).toBeGreaterThan(0);
    result.results.forEach(r => {
      if (r.success) {
        expect(r.volume).toBe(true);
        expect(r.frequencyData).toBe(true);
      }
    });
  });
});
...
test.describe("Utility Functions", () => {
  ...
  test("should handle getDevices with multiple filter criteria", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { getDevices } = await import("/dist/index.js");
      ...
      return {
        ...
        filterMatches: filteredDevices.every(d =>
  ...
  test("should handle getDevices with non-existent filter values", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { getDevices } = await import("/dist/index.js");
  ...
  test("should handle getMediaStream with various constraints", async ({ page }) => {
    const result = await page.evaluate(async () => {
  ...
  test("should handle getMediaStream with invalid constraints", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { getMediaStream } = await import("/dist/index.js");
  ...
  test("should handle concurrent getMediaStream calls", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const promises = Array(concurrentCalls).fill().map(() =>
        getMediaStream({ audio: true, video: false })
      );
      ...
      try {
        ...
        return {
          ...
          allValidStreams: results.every(r => r.isMediaStream && r.audioTracksCount > 0),
          results,
        };
      } catch (error) {
        return {
  ...
  test("should handle getDevices with undefined filter values", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { getDevices } = await import("/dist/index.js");
  ...
  test("should handle getMediaStream with default constraints", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { getMediaStream } = await import("/dist/index.js");
